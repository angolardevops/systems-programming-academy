// Package main — boollang: the language gains short-circuit boolean logic.
//
...
// `if`: `and` skips its right side when the left is false, `or` skips it when the
// left is true. Booleans are integers (1 = true, 0 = false; any nonzero is
// truthy). The operators are keywords the parser recognizes by name, below
// comparison in precedence: or < and < not < comparison. Library and command
...
// orExpr := andExpr ('or' andExpr)*    — loosest boolean level
...
// notExpr := 'not' notExpr | comparison   — 'not' binds tighter than and/or
